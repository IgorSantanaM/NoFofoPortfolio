  ...
  return <>
    ...
    <header className="site-header" id="inicio">
      ...
      <nav id="main-navigation" className={menuOpen ? 'menu-open' : ''} aria-label="Navegação principal">
        <a href="#criacao" onClick={() => setMenuOpen(false)}>Criações</a><a href="#sobre" onClick={() => setMenuOpen(false)}>Nosso fio</a><a href="#encomenda" onClick={() => setMenuOpen(false)}>Como encomendar</a><a className="mobile-contact" href="#contato" onClick={() => setMenuOpen(false)}>Contato</a>
      </nav>
    ...
    <main id="conteudo">
      <section className="hero section-wrap" aria-labelledby="hero-heading">
        ...
        <div className="hero-art"><div className="orbit orbit-one" aria-hidden="true"/><div className="orbit orbit-two" aria-hidden="true"/><span className="art-caption">CADA PONTO, UMA HISTÓRIA</span><div className="logo-disc"><img src={assetUrl('logo-no-fofo.jpeg')} alt="Nó Fofo: um coração feito de fios" width="1254" height="1254" fetchPriority="high"/></div><span className="art-note">um fio de carinho<br/><em>feito só para você</em></span><span className="art-star" aria-hidden="true">✳</span><span className="art-index">ESTÚDIO DE CROCHÊ<br/>FEITO À MÃO, COM CALMA</span></div>
      </section>
      ...
      <section id="criacao" className="section-wrap portfolio" aria-labelledby="portfolio-heading">
        <div data-reveal><p className="eyebrow">01 / NOSSAS CRIAÇÕES</p><div className="section-heading"><h2 id="portfolio-heading">Afeto em <em>cada ponto.</em></h2><p>Pequenas criações para grandes significados.<br/>Explore as possibilidades do feito à mão.</p></div></div>
        ...
        <div className="catalog-footer"><span>Imaginou algo diferente?</span><button type="button" className="text-link" onClick={() => openContact()}>Vamos criar do seu jeito <span aria-hidden="true">↗</span></button></div>
      </section>
      <section id="sobre" className="about-section" aria-labelledby="about-heading"><div className="section-wrap about-grid" data-reveal><div className="about-art"><div className="about-mark" aria-hidden="true">nó.</div><div className="about-line" aria-hidden="true"/><p>O fio é só o começo.<br/><em>O carinho é o que fica.</em></p><span className="about-art-index">A ESSÊNCIA NÓ FOFO</span></div><div className="about-copy"><p className="eyebrow">02 / NOSSO FIO</p><h2 id="about-heading">Tem um pouco<br/>de amor em<br/><em>tudo que é feito à mão.</em></h2><p>A Nó Fofo é um encontro entre fios, ideias e afeto. Um lugar para dar forma à delicadeza — seja em um amigurumi que faz sorrir ou em um detalhe que deixa a casa mais sua.</p><p>O que nos inspira são as pequenas coisas: uma cor que lembra alguém, um presente com significado, a vontade de cuidar. Ponto a ponto, essas ideias encontram um jeito de existir.</p><div className="values"><span>Feito com cuidado</span><span>Cheio de significado</span></div></div></div></section>
      <section id="encomenda" className="section-wrap order-section" aria-labelledby="order-heading"><div data-reveal><p className="eyebrow">03 / DO PRIMEIRO OI AO ÚLTIMO PONTO</p><div className="section-heading"><h2 id="order-heading">Sua ideia.<br/><em>Nossos pontos.</em></h2><p>Uma criação especial começa com uma conversa. Veja o caminho para tirar a sua ideia do papel.</p></div></div><ol className="steps">{[
      ...
      <section id="contato" className="contact-section" aria-labelledby="contact-heading"><div className="section-wrap contact-inner" data-reveal><p className="eyebrow">04 / VAMOS PUXAR ESSE FIO?</p><h2 id="contact-heading">Qual carinho vamos<br/><em>criar juntos?</em></h2><p>Para presentear alguém. Para deixar a casa mais sua.<br/>Ou simplesmente para você.</p>{whatsapp ? <a className="button contact-cta" href={whatsappUrl} target="_blank" rel="noopener noreferrer">Encomendar pelo WhatsApp <span aria-hidden="true">↗</span></a> : <button className="button contact-cta" type="button" onClick={() => openContact()}>Planejar minha encomenda <span aria-hidden="true">↗</span></button>}<div className="contact-channels">{whatsapp ? <a href={whatsappUrl} target="_blank" rel="noopener noreferrer">WhatsApp {whatsappDisplay} ↗</a> : contactAvailable ? <span>Vamos conversar sobre sua ideia</span> : <span>Canais de contato em preparação</span>}{siteConfig.instagramUrl && <><span className="channel-separator" aria-hidden="true">·</span><a href={siteConfig.instagramUrl} target="_blank" rel="noopener noreferrer">Acompanhe no Instagram ↗</a></>}</div><span className="contact-star" aria-hidden="true">✳</span></div></section>
    </main>
