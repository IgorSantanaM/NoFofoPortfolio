...
function App() {
  ...
  return <>
    ...
    <main id="conteudo">
      ...
      <section id="contato" className="contact-section" aria-labelledby="contact-heading"><div className="section-wrap contact-inner" data-reveal><h2 id="contact-heading">Qual carinho vamos<br/><em>criar juntos?</em></h2><p>Para presentear alguém. Para deixar a casa mais sua.<br/>Ou simplesmente para você.</p>{whatsapp ? <a className="button contact-cta" href={whatsappUrl} target="_blank" rel="noopener noreferrer">Encomendar pelo WhatsApp <span aria-hidden="true">↗</span></a> : <button className="button contact-cta" type="button" onClick={() => openContact()}>Planejar minha encomenda <span aria-hidden="true">↗</span></button>}<div className="contact-channels">{whatsapp ? <a href={whatsappUrl} target="_blank" rel="noopener noreferrer">WhatsApp {whatsappDisplay} ↗</a> : contactAvailable ? <span>Vamos conversar sobre sua ideia</span> : <span>Canais de contato em preparação</span>}{siteConfig.instagramUrl && <><span className="channel-separator" aria-hidden="true">·</span><a href={siteConfig.instagramUrl} target="_blank" rel="noopener noreferrer">Acompanhe no Instagram ↗</a></>}</div><span className="contact-star" aria-hidden="true">✳</span></div></section>
    </main>
